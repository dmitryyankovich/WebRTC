(function (global, undefined) {
    ...
    function createConn(stream, result) {
        if (result === 'Connected') {
            if (stream || redirConn) {
                ...
                hub.server.send(JSON.stringify({ redir: true }), roomId);
            }
        }
    }
        ...
        remoteDataChannel.onmessage = function (message) {
            ...
            switch (mess.type) {
                case 'file':
                    ...
                    receivedIndexArray[mess.chunkNumber] = mess.chunkNumber;
                    if (mess.last === true) {
                        console.log('last');
                    }
    ...
    hub.client.newMessage = function (data) {
        var message = JSON.parse(data), cand;
        if (message.redir) {
            redirConn = true;
            createConn(myMediaStream, 'Connected');
        } else {
            var connection = myConnection || createConnection();
            if (message.sdp) {
                sdpReceived = true;
